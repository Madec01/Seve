...
export const STEPS = [
  ...
  },
  {
    ...
    onEnter(run) {
      // Tout ce qui est planté mûrit. S'il n'existe aucune paire voisine de
      // degrés différents, on en pose une près du joueur, où qu'il soit.
      const f = run.field;
      const ripe = f.tiles.filter((x) => x.plant && !x.plant.wilted);
      for (const x of ripe) { x.plant.growth = 1; x.plant.ripe = true; x.plant.pop = 1; }
      if (hasNeighbourPair(f, ripe)) return;
      const pc = run.player.col, pr = run.player.row;
      const usable = (t) => t && t.terrain !== 'void' && t.terrain !== 'water' && !t.fissure
        && !(t.c === pc && t.r === pr);
      const candidates = f.tilesInRadius(pc, pr, 2.5).filter(usable)
        .sort((a, b) => Math.hypot(a.c - pc, a.r - pr) - Math.hypot(b.c - pc, b.r - pr));
      for (const t1 of candidates) {
        const t2 = f.neighbors(t1.c, t1.r).find((n) => usable(n) && !n.plant);
        if (!t2) continue;
        for (const [t, deg] of [[t1, 'I'], [t2, 'III']]) {
